...
def read_process_data(filename):
    ...
    fd = open(filename, 'r')
    while cpu_count < 60 or mem_count < 60:
        line = fd.readline()
        ...
        if n[0] == 'CPU':
            if cpu_count >= 60:
                continue
            cpu_count += 1
            cpu.append(float(fields[7]))
        elif n[0] == 'MEM':
            if mem_count >= 60:
                continue
            mem_count += 1
            mem.append(float(fields[7]))

    return cpu, mem


if __name__ == "__main__":
    gnet_cpu, gnet_mem = read_process_data('gnet.cpu')
    go124_cpu, go124_mem = read_process_data('go124.cpu')
